...
public class PersonaDAO implements CrudPaginadoInterface<Persona> {
    ...
    public List<Persona> listar(String texto,int totalPorPagina,int numPagina) {
        ...
        try {
            ps=CON.conectar().prepareStatement("SELECT p.id, p.tipo_persona, p.nombre, p.tipo_documento, p.num_documento, p.direccion, p.telefono, p.email, p.activo FROM persona p WHERE p.nombre LIKE ? ORDER BY p.id ASC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY");
    ...
    public List<Persona> listarTipo(String texto,int totalPorPagina,int numPagina, String tipoPersona) {
        List<Persona> registros=new ArrayList();
        try {
            ps=CON.conectar().prepareStatement("SELECT p.id, p.tipo_persona, p.nombre, p.tipo_documento, p.num_documento, p.direccion, p.telefono, p.email, p.activo FROM persona p WHERE p.nombre LIKE ? AND tipo_persona=? ORDER BY p.id ASC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY");
            ps.setString(1,"%" + texto +"%");
